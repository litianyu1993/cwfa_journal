...
def get_hankel(train_file, l, pt, option, lr = 0.1, epochs = 1000,  lrows = 1, lcolumns = 1, exact_length = False, version ='classic'):
    ...
    print(mdl.length)
    mdl.learning(x, y, scheduler_params, lr=lr, epochs=epochs, verbose=True, weight_decay=0)
    H_l = mdl.get_tt()
    with open('sgdv2_hl', 'wb') as f:
        pickle.dump(H_l, f)
    return H_l
...
if __name__ == '__main__':
    ...
    # print(load)
    if not load:
        H_l = get_hankel(train_file, l, pt, option, lr=0.1, epochs=epochs, lrows=100, lcolumns=100, exact_length=False, version=version)
        option['length'] = 2*l
        H_2l = get_hankel(train_file, 2*l, pt, option, lr=0.01, epochs=epochs, lrows=100, lcolumns=100, exact_length=False, version=version)
        option['length'] = 2 * l + 1
        H_2l1 = get_hankel(train_file, 2*l+1, pt, option, lr=0.01, epochs=epochs, lrows=100, lcolumns=100, exact_length=False,
                         version=version)

    else:
        with open('sgdv2_hl', 'rb') as f:
            H_l = pickle.load(f)
        with open('sgdv2_h2l', 'rb') as f:
            H_2l = pickle.load(f)
        with open('sgdv2_h2l1', 'rb') as f:
            H_2l1 = pickle.load(f)
